...
async function getExhibition(path: string): Promise<DrupalExhibition | null> {
  try {
    ...
    const data = await client.raw(GET_EXHIBITION_BY_PATH, { path })
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!item) {
    return { title: 'Exhibition Not Found | Arts Foundation' }
  }
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).location && (
                    <div>
                      <dt className="text-sm text-gray-500">Gallery/Location</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).location}</dd>
                    </div>
                  )}
                  {(item as any).curator && (
                    <div>
                      <dt className="text-sm text-gray-500">Curator</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).curator}</dd>
                    </div>
                  )}
