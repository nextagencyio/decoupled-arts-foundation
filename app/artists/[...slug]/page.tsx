...
import { GET_ARTIST_BY_PATH } from '@/lib/queries'
import { DrupalArtist } from '@/lib/types'
...
interface ArtistByPathData {
  route: {
    entity: DrupalArtist
  } | null
}

async function getArtist(path: string): Promise<DrupalArtist | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<ArtistByPathData>({
      query: GET_ARTIST_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/artists/${slug.join('/')}`
  const item = await getArtist(path)

  if (!item) {
    return { title: 'Artist Not Found | Arts Foundation' }
  }

  return {
    title: `${item.title} | Arts Foundation`,
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).artistType && (
                    <div>
                      <dt className="text-sm text-gray-500">Artist Type</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).artistType}</dd>
                    </div>
                  )}
                  {(item as any).residencyDates && (
                    <div>
                      <dt className="text-sm text-gray-500">Residency Dates</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).residencyDates}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
